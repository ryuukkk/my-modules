...
def save(filename, cars):
    ...
    except (pickle.UnpicklingError, EnvironmentError) as err:
        print(err)
        sys.exit(1)
...
class CarRegServer(socketserver.ThreadingMixIn, socketserver.TCPServer): pass
...
class RequestHandler(socketserver.StreamRequestHandler):
    ...
    def handle(self) -> None:
        ...
        size = self.rfile.read(SizeStruct.size)
        size = SizeStruct.unpack(size)[0]
...
def main():
    ...
    server = None
    try:
        server = CarRegServer(('', 9653), RequestHandler)
